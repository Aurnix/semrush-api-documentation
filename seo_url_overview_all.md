# SEO API — URL Overview (all databases)

**Price:** 10 API units per line
**Price (historical data):** 50 API units per line

Provides live or historical data on a URL's keyword rankings in both organic and paid search across all regional databases.
...
|---|---|---|
| `type` | Yes | Report type. Value: `url_ranks` |
...
| `export_columns` | No | Comma-separated columns. Default: `Db,Rk,Dn,Or,Ot,Oc,Ad,At,Ac,Sh,Sv`. Available: `Db,Dt,Dn,Rk,Or,Ot,Oc,Ad,At,Ac,Sh,Sv,FKn,FPn,Sr,Srb,St,Stb,Sc,Srn,Srl`. See [Columns](seo_api_overview.md#export-columns). |
...
```
https://api.semrush.com/?key=API_KEY&type=url_ranks&export_columns=Db,Dn,Or,Ot,Oc,Ad,At,Ac,Sh,Sv&url=https%3A%2F%2Fwww.ebay.com%2Fsignin%2F&database=us
```
...
us;ebay.com;507;394412;89819;1;15;2;0;0
```
